...
### INTAKE_LEAN
(lean mode only) Orchestrator delegates initial artifact creation to SpecAgent (with lean flag).
...
Trigger: Orchestrator enters this state automatically after DESIGN completes.
...
- Design specs (if Designer was involved)
...
2. After user response, classify the outcome:
   ...
   - **No answer / invalid answer**: treat as missing mandatory gate decision. Re-ask up to 3 times, then enter `BLOCKED` if still unresolved.
   - **Explicit defer/cancel by user**: mark `cancelled` with explicit reason and enter `BLOCKED` (`mandatory_user_decision_missing`).
3. **Gate verification**: Re-read `status.json` and confirm `UD-APPROVE-DESIGN` has `status: answered` AND `answer` starts with `"approved"`. If the answer starts with `"changes-requested"`, do NOT proceed.
...
The Orchestrator sets `current_state: REVIEW_STRATEGY` in `status.json`.
...
Gate: User explicitly chooses a strategy. The choice is stored as a `user_decisions` entry in `status.json` (decision_id format: `UD-REVIEW-STRATEGY`).

Orchestrator MUST follow the full ASK_USER protocol from CONTRACT.md, including:
...
5. **During IMPLEMENT_LOOP**: Before dispatching Reviewer/QA/Security, Orchestrator MUST re-read `UD-REVIEW-STRATEGY` from `status.json` to confirm the chosen strategy. NEVER rely on in-memory state alone — always verify from disk.
...
2. Reviewer reviews (with ALL session-changed files, not just current task).
...
2. `session_changed_files` in `task` MUST list ALL files changed during the session (by any agent) with `change_type` per entry.
...
5. After the combined review passes, all tasks are promoted to `completed` by Orchestrator, and workflow proceeds directly to INTEGRATE.

#### Full-scope context rule for Reviewer
Orchestrator MUST provide Reviewer with the list of ALL files changed during the entire session (not just the current task's files) via a `session_changed_files` field in the `task` object. This is separate from `context_files`.
...
**Final review (comprehensive)**: Reviewer reads all non-deleted files from `session_changed_files` in full and reviews deleted files via diff only.

#### Cross-task final review (mandatory in full mode, per-batch strategy only)
This step applies ONLY when the review strategy is **per-batch**. In **single-final** mode, the combined review pass already serves as the cross-task final review — do NOT run it a second time.
...
2. `session_changed_files` in `task` MUST list ALL files changed during the session.
3. Reviewer reads the full content of every non-deleted file in `session_changed_files` and applies the full checklist with focus on cross-cutting concerns.
...
- If `UD-APPROVE-DESIGN.answer` starts with `"changes-requested:"`, consult `gate_tracking.APPROVE_DESIGN.correction_status` before dispatching corrections. If the field is missing/null (or `gate_tracking` is absent), treat it as `"none"`:
  - `none` -> set `correction_status` to `queued` (backward compatibility), then continue.
...
Note: `APPROVE_DESIGN` and `REVIEW_STRATEGY` are NOT `ASK_USER` — they are distinct workflow states with their own `current_state` values.
Mechanism: Orchestrator uses `ask_questions` tool (NEVER plain text that ends the turn).
Protocol: follow the canonical ASK_USER protocol in `CONTRACT.md`.
Resume: only after CONTRACT validation passes, then return to the state recorded in `state_context` of the resolved decision.
...
- Deterministic security trigger: Security agent returns `NEEDS_DECISION` -> Orchestrator MUST enter ASK_USER
...
In **lean mode**, Orchestrator runs acceptance_checks commands directly instead of dispatching Integrator.
...
**Lean mode exception**: In lean mode, only Coder, Reviewer, and (conditionally) QA and Security are mandatory. SpecAgent is used for INTAKE_LEAN artifact creation. Architect, Planner, Designer, Integrator, Docs, and Researcher are skipped. Orchestrator handles INTEGRATE checks directly and creates `report.md` directly.
